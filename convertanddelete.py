...
def save_corrected_slices(nii_file):
    ...
    axial_img.save(os.path.join(os.path.dirname(nii_file), 'axial_slice.png'))  # Save the axial slice image as PNG.
    ...
    coronal_img.save(os.path.join(os.path.dirname(nii_file), 'coronal_slice.png'))  # Save the coronal slice image as PNG.
    ...
    sagittal_img.save(os.path.join(os.path.dirname(nii_file), 'sagittal_slice.png'))  # Save the sagittal slice image as PNG.

    # Delete the NIfTI file after processing
    os.remove(nii_file)  # Remove the NIfTI file.
    ...
    for root, _, files in os.walk(patient_folder):  # Walk through the directory, returning root, directories, and files.
        for file in files:  # Loop through the list of files
            if file.endswith('.nii'):  # Check if the file is a NIfTI file
                ...
                save_corrected_slices(nii_file)  # Call the function to process and save slices from the NIfTI file.
                print(f'Converted {nii_file} to PNG images and deleted the original NIfTI file.')  # Print a message indicating the conversion and deletion.
...
# Example usage
data_dir = '/mnt/Game Library 1/Dataset-backup'  # Define the path to the main data directory containing patient folders.
